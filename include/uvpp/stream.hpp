...
public:
    ...
    ~stream()
    {
       std::cout << " delete stream class......" << std::endl;
    }
...
public:
    ...
    bool write(const char* buf, int len, void* data, std::function<void(void* data, error status)> callback)
    {
        ...
        req->data = data;
        return uv_write(req, handle<HANDLE_T>::template get<uv_stream_t>(), bufs, 1, [](uv_write_t* req, int status)
        {
            std::unique_ptr<uv_write_t> reqHolder(req);
            callbacks::invoke<decltype(callback)>(req->handle->data, uvpp::internal::uv_cid_write, req->data, error(status));
        }) == 0;
    }
